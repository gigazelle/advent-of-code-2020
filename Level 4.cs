...
namespace Advent_of_Code
{
    class Level_4
    {
        ...
        {
            string[] passports = {"eyr:2033 hgt:177cm pid:173cm ecl:utc byr:2029 hcl:#efcc98 iyr:2023",
...
"hgt:183cm cid:187 byr:2019 ecl:xry iyr:2013 pid:164cm hcl:#18171d eyr:2021"};
            ...
            {
                if (passports[i].IndexOf("byr") >= 0 && passports[i].IndexOf("iyr") >= 0 && passports[i].IndexOf("eyr") >= 0 && passports[i].IndexOf("hgt") >= 0 && passports[i].IndexOf("hcl") >= 0 && passports[i].IndexOf("ecl") >= 0 && passports[i].IndexOf("pid") >= 0)
                {
                    tempVal = passports[i].Substring(passports[i].IndexOf("byr:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    if (Int32.Parse(tempVal) > 2002 || Int32.Parse(tempVal) < 1920) continue;
                    tempVal = passports[i].Substring(passports[i].IndexOf("iyr:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    if (Int32.Parse(tempVal) > 2020 || Int32.Parse(tempVal) < 2010) continue;
                    tempVal = passports[i].Substring(passports[i].IndexOf("eyr:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    if (Int32.Parse(tempVal) > 2030 || Int32.Parse(tempVal) < 2020) continue;
                    tempVal = passports[i].Substring(passports[i].IndexOf("hgt:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    string cmorin = tempVal.Substring(tempVal.Length - 2);
                    if (cmorin != "cm" && cmorin != "in") continue;
                    if (cmorin == "cm")
                    {
                        if (Int32.Parse(tempVal.Substring(0, tempVal.Length - 2)) > 193 || Int32.Parse(tempVal.Substring(0, tempVal.Length - 2)) < 150) continue;
                    }
                    else
                    {
                        if (Int32.Parse(tempVal.Substring(0, tempVal.Length - 2)) > 76 || Int32.Parse(tempVal.Substring(0, tempVal.Length - 2)) < 59) continue;

                    }
                    tempVal = passports[i].Substring(passports[i].IndexOf("hcl:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    if (!Regex.Match(tempVal, "^#(?:[0-9a-fA-F]{3}){1,2}$").Success) continue;
                    tempVal = passports[i].Substring(passports[i].IndexOf("ecl:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    if(tempVal != "amb" && tempVal != "blu" && tempVal != "brn" && tempVal != "gry" && tempVal != "grn" && tempVal != "hzl" && tempVal != "oth") continue; 
                    tempVal = passports[i].Substring(passports[i].IndexOf("pid:"));
                    tempVal = tempVal.Split(" ")[0];
                    tempVal = tempVal.Substring(4);
                    if(tempVal.Length != 9) continue;

                    //Console.WriteLine(cmorin);
