...
namespace Advent_of_Code
{
    class Level_2_1
    {
        ...
        {
            string[] passArray = {"5-10 b: bhbjlkbbbbbbb",
"3-4 j: hjvj",
"8-9 p: pmljtsttp",
...
"8-9 v: vvvvvvvvvg"};
            ...
            {
                passArray[i] = passArray[i].Replace(": ", "-");
                passArray[i] = passArray[i].Replace(" ", "-");
                string[] dividedValue = passArray[i].Split("-");
                //Console.WriteLine(passArray[i].ToString());
                //Array.ForEach(dividedValue, Console.WriteLine);
                int minNum = Int32.Parse(dividedValue[0]);
                int maxNum = Int32.Parse(dividedValue[1]);
                string testLetter = dividedValue[2];
                string password = dividedValue[3];
                /*int passwordTestCount = password.Split(testLetter).Length - 1;
                Console.WriteLine(passwordTestCount.ToString());
                if (passwordTestCount >= minNum && passwordTestCount <= maxNum)
                {
                    goodPassword++;
                }*/
                char firstTest = password[minNum-1];
                char secondTest = password[maxNum-1];
                char testLetterChar = char.Parse(testLetter);
                if((firstTest == testLetterChar && secondTest != testLetterChar) || (firstTest != testLetterChar && secondTest == testLetterChar)){
                    goodPassword++;
                }
            }
            Console.WriteLine(goodPassword);
        }
    }
}